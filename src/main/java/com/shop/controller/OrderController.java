...
public class OrderController {
    ...
    public @ResponseBody ResponseEntity order(@RequestBody @Valid OrderDto orderDto ,
        ...
        // @RequsetBody : HTTP 요청의 본문 body에 담긴 내용을 자바 객체로 전달
        ...
        // 현재 로그인 정보를 얻기 위해서 @Controller 어노테이션이 선언된 클래스에서 메소드 인자로 principal 객체를 넘겨 줄 경우
        // 해당 객체에 직접 접근할 수 있습니다. principal 객체에서 현재 로그인한 회원의 이메일 정보를 조회 합니다.
        String email = principal.getName();
        Long orderId;

        try {
            orderId = orderService.order(orderDto , email); // 화면으로부터 넘어오는 주문 정보와 회원의 이메일 정보를 이용하여 주문 로직을 호출합니다.
        }catch (Exception e) {
