...
public class CartService {
    ...
    public Long addCart(CartItemDto cartItemDto , String email) {
        ...
        if(savedCartItem != null) {
            savedCartItem.addCount(cartItemDto.getCount()); // 장바구니에 이미 있던 상품일 경우 기존 수량에 현재 장바구니에 담을 수량 만큼 더해줍니다.
    ...
    public List<CartDetailDto> getCartList(String email) {
        ...
        Cart cart = cartRepository.findByMemberId(member.getId()); // 현재 로그인한 회원의 장바구니 엔티티를 조회합니다
        if(cart == null){ // 장바구니에 상품을 한 번도 안 담았을 경우 장바구니 엔티티가 없으므로 빈 리스트를 반환합니다.
    ...
    @Transactional(readOnly = true)
    public boolean validCartItem(Long cartItemId , String email) {
        ...
        // 현재 로그인한 회원과 장바구니 상품을 저장한 회원이 다를 경우 false를, 같으면 true를 반환합니다.
    ...
    public Long orderCartItem(List<CartOrderDto> cartOrderDtoList , String email) {
        ...
        Long orderId = orderService.orders(orderDtoList , email); // 장바구니에 담은 상품을 주문하도록 주문 로직을 호출합니다.

        for(CartOrderDto cartOrderDto : cartOrderDtoList) { // 주문한 상품들을 장바구니에서 제거합니다.
            CartItem cartItem = cartItemRepository.findById(cartOrderDto.getCartItemId())
                    .orElseThrow(EntityNotFoundException::new);
            cartItemRepository.delete(cartItem);
        }
